...
#Model 1:
    ...
    # Transport: ersstArc.win 
        ...
        arrayname<-"Model1.array"
              filename<-"p2TsuFfall"; Fname="F.S3.ParrF"; FW.name="Ffall"
          #   filename<-"p2Fsu"; Fname="F.S2.ParrSu"; FW.name="Fsu"
          #   filename<-"c2TsuFsu"; Fname="F.S2.ParrSu"; FW.name="Fsu"
      
              load(file=paste("Input files/param.array.1K",filename,"May2020.calib.step2",sep="."),verbose=T) #param.array.1K
          #   load(file=paste("Input files/param.array.1K",filename,"May2020.calib.step2","arcspr.cuispr",sep="."),verbose=T) #param.array.5K
          #   load (file="output/spawn_recruit_output/param.array.5K.p2TsuFfall.May2020.calib.step1",verbose=T) #param.array.5K
                gompertz.param<-param.array.1K
...
#       pop<-creeks<-c("bearvalley","big","camas","loon","marsh","secesh","sulphur","valley")
        ...
        nsim<-length(pop)*length(ocean)*length(climate)*length(quant)#;nsim
        scenario<-length(ocean)*length(climate)*length(quant);scenario
        scenario.name<-vector("character",length=scenario)
     cnt.scenario=0
        for(l in 1:length(ocean)){
            for(cc in 1:length(climate)){ 
            for(qq in 1:length(quant)){ 
                 cnt.scenario=cnt.scenario+1
               scenario.name[cnt.scenario]=paste(climate[cc],quant[qq],ocean[l],sep=".");scenario.name
            }}}
...
#Input files-------------------
    ...
    #SARs    
         load("Input files/Model1.Model2.SAR100.Rdata",verbose=TRUE) 
...
# Set up temporary arrays to store data during model runs-----------
 ...
 #   mean.s1=mean.Strib=mean.inriver=mean.pt=mean.pt.sar=mean.pt.sup=mean.main=mean.s3=mean.sar=mean.sarI=mean.sarT=mean.sup=mean.spawn = mean.pop.fac=geomean.spawn = median.spawn=final.spawn=CI05.spawn=CI95.spawn=Nyr25.spawn=Nyr100.spawn = array(0,runs) #summaries of each run
# Arrays to save output-----------
      ...
      response<-c("yrQET50","Mean2020","Mean2040","Mean2060","Mean2080","Meanallyears")
            allparam<-c( names(param.marI),names(param.marT),dimnames(gompertz.param)[[2]],response)# ;allparam
...
for(k in 1:length(pop)){ # loop across populations
  ...
  for (j in 1:runs){
    # ifelse(j<1001, jj<-j,
    #        jj<-1+j%%length(gompertz.param[,1,1]));jj #this gives you the remainder of dividing runs by length of param list
    #     jj.sar<-jj
   jj<-sample(length(gompertz.param[,1,1]),1)
   jj.sar<-sample(length(sarI_rqys[1,1,1,]),1)
        param<-as.list(gompertz.param[jj,,pop[k]])#;unlist(param)
...
#Initialize all age classes 
     N[1:5] = c(0,0,0,0,0)	
        for (i in 1:5){ 
          xx<-sample(1:dim(sc_envProj_rqvys)[5],1)
